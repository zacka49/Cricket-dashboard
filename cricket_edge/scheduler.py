...
class BackgroundScheduler:
    ...
    def _ensure_state_row(self) -> None:
        self.db.execute("INSERT OR IGNORE INTO scheduler_state(id, updated_at) VALUES (1, ?)", (utc_now(),))
    ...
    def run_forever(self) -> None:
        while True:
            try:
                ...
                self.db.log_event("scheduler", f"Tick failed: {exc}", {"error": str(exc)})
            time.sleep(SETTINGS.scheduler_tick_seconds)

    def tick(self) -> dict[str, Any]:
        state = self.db.query_one("SELECT * FROM scheduler_state WHERE id = 1")
        today = date.today().isoformat()

        if state["last_morning_run_date"] != today:
            self.app.morning_run()
            self.db.execute("UPDATE scheduler_state SET last_morning_run_date = ? WHERE id = 1", (today,))
        ...
        if self._retrain_due(state):
            ...
            # heartbeat below from updating -- "alive" means the scheduler is
            # ticking, not that every step succeeded.
            try:
                self.app.train_week3_models()
                self.db.execute(
                    "UPDATE scheduler_state SET last_retrain_at = ?, last_retrain_match_count = ? WHERE id = 1",
                    (utc_now(), self._match_count()),
                )
            except Exception as exc:  # noqa: BLE001
                self.db.log_event("scheduler", f"Retrain failed: {exc}", {"error": str(exc)})

        self.db.execute("UPDATE scheduler_state SET last_tick_at = ?, updated_at = ? WHERE id = 1", (utc_now(), utc_now()))
        self.db.log_event("scheduler", "Tick completed.", {})
    ...
    def _retrain_due(self, state: dict[str, Any]) -> bool:
        ...
        if hours_since >= SETTINGS.scheduler_retrain_interval_hours:
            return True
        new_matches = self._match_count() - int(state["last_retrain_match_count"] or 0)
        return new_matches >= SETTINGS.scheduler_retrain_new_match_threshold
...
def start_scheduler() -> BackgroundScheduler | None:
    ...
    thread = threading.Thread(target=scheduler.run_forever, daemon=True, name="background-scheduler")
    thread.start()
    return scheduler
