...
from cricket_edge.agents import (
    BetDecisionAgent,
    DataStewardAgent,
    MarketWatchAgent,
...
def _insert_agent_decision(db: Database, agent_name: str, decision: str) -> None:
    db.execute(
        """
...
def test_data_steward_reports_healthy_with_data(tmp_path: Path) -> None:
    ...
    result = DataStewardAgent(db).run()

    assert result["decision"] == "healthy"
    assert result["issues"] == []
    row = db.query_one("SELECT * FROM agent_decisions WHERE agent_name = 'data_steward' ORDER BY id DESC LIMIT 1")
    assert row["decision"] == "healthy"


def test_data_steward_reports_needs_attention_when_empty(tmp_path: Path) -> None:
    db = Database(tmp_path / "x.sqlite3")
    db.init_schema()
...
# --- BetDecisionAgent: evaluate()/execute() split ---------------------------
...
def test_evaluate_proposes_without_placing_a_bet(tmp_path: Path) -> None:
    ...
    decisions = BetDecisionAgent(db).evaluate()
    ...
    recorded = db.query("SELECT * FROM agent_decisions WHERE agent_name = 'bet_decision_agent'")
...
def test_run_wrapper_places_bet_end_to_end_when_nothing_vetoes_it(tmp_path: Path) -> None:
    ...
    reviewed = BetDecisionAgent(db).run()
...
def test_oversight_vetoes_all_bets_when_data_steward_needs_attention(tmp_path: Path) -> None:
    db = Database(tmp_path / "x.sqlite3")
    db.init_schema()
    fixture_id = _insert_fixture(db, "Alpha", "Beta")
    _insert_agent_decision(db, "data_steward", "needs_attention")
    proposal = {"fixture_id": fixture_id, "decision": "paper_bet", "stake": 8.0, "confidence": 0.7, "decision_id": 999}

    reviewed = PortfolioOversightAgent(db).review([proposal])

    assert reviewed[0]["oversight"]["status"] == "vetoed"
    veto_rows = db.query("SELECT * FROM agent_decisions WHERE agent_name = 'portfolio_oversight_agent'")
...
def test_oversight_vetoes_overflow_beyond_portfolio_cap(tmp_path: Path) -> None:
    db = Database(tmp_path / "x.sqlite3")
    db.init_schema()
    fixture_id = _insert_fixture(db, "Alpha", "Beta")
    proposals = [
    ...
    reviewed = PortfolioOversightAgent(db, portfolio_cap_fraction=0.01).review(proposals)
...
def test_oversight_approves_proposals_under_the_cap(tmp_path: Path) -> None:
    db = Database(tmp_path / "x.sqlite3")
    db.init_schema()
    fixture_id = _insert_fixture(db, "Alpha", "Beta")
    ...
    assert reviewed[0]["oversight"]["status"] == "approved"
    assert db.query("SELECT * FROM agent_decisions WHERE agent_name = 'portfolio_oversight_agent'") == []


def test_oversight_marks_non_bet_decisions_not_applicable(tmp_path: Path) -> None:
    db = Database(tmp_path / "x.sqlite3")
    db.init_schema()
    fixture_id = _insert_fixture(db, "Alpha", "Beta")
...
def test_market_watch_cashes_out_on_material_shorten(tmp_path: Path) -> None:
    db = Database(tmp_path / "x.sqlite3")
    db.init_schema()
    fixture_id = _insert_fixture(db, "Alpha", "Beta")
    ...
    actions = MarketWatchAgent(db).run()
...
def test_market_watch_cashes_out_on_material_drift(tmp_path: Path) -> None:
    db = Database(tmp_path / "x.sqlite3")
    db.init_schema()
    fixture_id = _insert_fixture(db, "Alpha", "Beta")
    bet_id = _place_open_bet(db, fixture_id, "Alpha", odds=2.0)
...
def test_market_watch_holds_on_small_move(tmp_path: Path) -> None:
    db = Database(tmp_path / "x.sqlite3")
    db.init_schema()
    fixture_id = _insert_fixture(db, "Alpha", "Beta")
    bet_id = _place_open_bet(db, fixture_id, "Alpha", odds=2.0)
...
def test_daily_briefing_mentions_top_edges_and_records_a_decision(tmp_path: Path) -> None:
    ...
    payload = ReportWriterAgent(db).daily_briefing()
    ...
    row = db.query_one("SELECT * FROM agent_decisions WHERE agent_name = 'report_writer_agent' ORDER BY id DESC LIMIT 1")
